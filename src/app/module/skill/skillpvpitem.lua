local skillpvpitem = class("skillpvpitem", function()
		local layout = ccui.Layout:create()
		layout:setContentSize(cc.size(616,138))
		return layout
	end)
function skillpvpitem:ctor()
	if(not skillpvpitem.view) then
		skillpvpitem.view = ResourceManager:widgetFromJsonFile("ui/skillpvpitem.json")
		skillpvpitem.view:retain()
	end	
	self.view = skillpvpitem.view:clone()
...
function skillpvpitem:updateData(locked,index,skillid)
	self.index = index
...
function skillpvpitem:onClick(sender,eventType)
	...
	Observer.sendNotification(skillmodule.UP_SKILL_ORDER_PVP,tempNode)
end
function skillpvpitem:onClickSkill(sender,eventType)
	if  eventType ~= TouchEventType.ended then
		return
	end
